...
class OrderDetailPage extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    Color statusColor;
    IconData statusIcon;
    if (status == "Selesai") { statusColor = Colors.green; statusIcon = Icons.check_circle; }
    else if (status == "Diproses" || status == "Menunggu") { statusColor = Colors.orange; statusIcon = Icons.access_time_filled; }
    else { statusColor = Colors.red; statusIcon = Icons.cancel; }
          ...
          children: [
              ...
              child: SingleChildScrollView(
                ...
                child: Column(
                  children: [
                        ...
                        children: [
                          Row(
                            children: [
                              ...
                              // Badge status disembunyikan jika sedang mode Job Offer (agar bersih sesuai referensi)
                              if (!isJobOffer)
                                Container(
                    ...
                    // MENU AKSI
                    if (isClient) ...[
                      _buildActionTile(Icons.person_outline, "Profile Pekerja", onTap: () => Navigator.push(context, MaterialPageRoute(builder: (context) => const ProfilePage()))),
                    ...
                    if (isClient) ...[
                      const SizedBox(height: 10),
                      _buildActionTile(Icons.rate_review_outlined, "Beri Ulasan ke Pekerja", onTap: () {}),
                    ],
            ...
            if (isJobOffer)
              Container(
                ...
                child: Row(
                  children: [
                    // TOMBOL TOLAK (Kiri, Biru Tua)
                    Expanded(
                      child: ElevatedButton(
                        onPressed: () => Navigator.pop(context), 
                        style: ElevatedButton.styleFrom(
                          backgroundColor: kPrimaryColor, // Biru Tua
                    ...
                    const SizedBox(width: 15),
                    // TOMBOL TERIMA (Kanan, Putih Outline)
                    Expanded(
                      child: OutlinedButton(
                        ...
                        child: const Text("Terima Pekerjaan", style: TextStyle(color: kPrimaryColor)), // Teks Biru
                      ),
                    ),
                  ],
                ),

...
import 'main.dart'; // Import main untuk akses isClient
import 'home_page_client.dart'; // Import halaman klien
...
class HomePage extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    // --- LOGIC SWITCHER ---
    // Jika login sebagai Klien, tampilkan halaman Klien
    if (MyApp.isClient) {
      return const HomePageClient();
    }

    // --- UI UNTUK PEKERJA (Kode Lama) ---
    return Scaffold(
  ...
  Widget _buildActiveOrderCard({required String clientName, required String address, required String service, required String type, required String date, required String package, required String imgAsset}) {
    return Container(
        ...
        children: [
          ...
          Row(
            ...
            children: [
              ...
              Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [_buildInfoRow("Tanggal Layanan", date), const SizedBox(height: 5), _buildInfoRow("Paket Layanan", package), const SizedBox(height: 8), const Text("Lihat Detail >", style: TextStyle(color: kPrimaryColor, fontWeight: FontWeight.bold, fontSize: 12))])),
            ],
          ),
        ],
      ),
  ...
  Widget _buildHistoryCard(String title, String date, String rating) {
    return Container(
      margin: const EdgeInsets.only(bottom: 10), padding: const EdgeInsets.all(15), decoration: BoxDecoration(color: Colors.white, borderRadius: BorderRadius.circular(12), boxShadow: [BoxShadow(color: Colors.grey.withOpacity(0.1), blurRadius: 5)]),
      child: Row(children: [Container(width: 45, height: 45, decoration: const BoxDecoration(color: Color(0xFFE0E0E0), shape: BoxShape.circle)), const SizedBox(width: 12), Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [Text(title, style: const TextStyle(fontWeight: FontWeight.bold, fontSize: 13)), Text(date, style: const TextStyle(color: Colors.grey, fontSize: 11))])), Column(crossAxisAlignment: CrossAxisAlignment.end, children: [const Row(children: [Icon(Icons.check_circle, color: Colors.green, size: 14), SizedBox(width: 4), Text("Selesai", style: TextStyle(color: Colors.green, fontSize: 11, fontWeight: FontWeight.bold))]), const SizedBox(height: 5), Container(padding: const EdgeInsets.symmetric(horizontal: 8, vertical: 2), decoration: BoxDecoration(color: Colors.lightBlue.shade50, borderRadius: BorderRadius.circular(8)), child: Row(children: [const Icon(Icons.star_border, size: 12, color: Colors.black), const SizedBox(width: 2), Text(rating, style: const TextStyle(fontSize: 11, fontWeight: FontWeight.bold))]))])]),
    );
  }
}